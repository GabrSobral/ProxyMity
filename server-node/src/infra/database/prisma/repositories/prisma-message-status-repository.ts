...
import { PrismaMessageStatusMapper } from '../mappers/prisma-message-status.mapper';
...
export class PrismaMessageStatusRepository implements MessageStatusRepository {
  ...
  async getUnreadMessagesStatusByUserId(userId: string, conversationId: string): Promise<Either<Error, MessageStatus[]>> {
    const unreadMessages = await this.prisma.messageStatus.findMany({
      where: {
        user_id: userId,
        conversation_id: conversationId,
  ...
  async getUnreadMessagesStatusCountByUserId(userId: string, conversationId: string): Promise<Either<Error, number>> {
    const unreadMessagesCount = await this.prisma.messageStatus.count({
      where: {
        user_id: userId,
        conversation_id: conversationId,
        AND: {
