...
<a
	...
	class="flex items-center gap-2 text-sm text-purple-300 underline-offset-2 hover:underline"
>
	<ArrowLeft /> Go back to sign-in
</a>

{#if errorAlertConfig}
	<WarningAlert errorMessage={errorAlertConfig} closeAlert={() => (errorAlertConfig = '')} />
{/if}

{#if step === 'form'}
	<Text size="md">
		We will send an e-mail to your inbox, type your e-mail account to receive the password reset
		link.
	</Text>
