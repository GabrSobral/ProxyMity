...
<form class="flex flex-col gap-4" on:submit|preventDefault={searchUser}>
   ...
   {#if searchedUser.user}
      ...
      >
         <Avatar.Image src="" username={`${searchedUser.user?.firstName} ${searchedUser.user?.lastName}`} />
   ...
   {#if selectedAccount}
      <Button type="button" onclick={createChat} class="ml-auto" disabled={currentUser?.id === searchedUser.user?.id}>
