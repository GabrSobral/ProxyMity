...
<Dialog.Root controlledOpen={isOpened}>
	<Dialog.Content
		class={twMerge(
			clsx(
				'flex h-[90%] w-fit min-w-[40rem] max-w-[95%] flex-col overflow-auto transition-all duration-500',
				{
	...
	>
		<div class="space-y-5">
			...
			<Dialog.Description>
				Seamlessly establish private or group conversations with ease. Whether you're fostering
				one-on-one connections or collaborating with teams, this intuitive modal lets you initiate
				real-time communication effortlessly.
			</Dialog.Description>
		</div>
				...
				{#each items as item (item)}
					...
					<button
						...
						class={cn(
							...
							'relative justify-start px-2 py-2 text-white hover:bg-transparent'
						)}
