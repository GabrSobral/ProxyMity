...
<div class="flex h-full gap-4">
	<form
		action=""
		class="flex min-w-[25rem] flex-col gap-4"
		onsubmit={(e) => {
			e.preventDefault();
			searchUser();
		}}
	>
		...
		<InputGroup let:Label let:ErrorMessage let:Input let:Wrapper>
			...
			<Wrapper className="w-full">
				<AtSign size={18} class="pointer-events-none absolute left-4 top-2/4 -translate-y-2/4" />
		...
		<ul class="flex flex-col gap-2">
			{#each usersData.users as user (user.id)}
				...
				<li
					...
					class={clsx(
						'flex cursor-pointer gap-4 rounded-md border border-gray-700 p-2 transition-all hover:bg-gray-600',
						{
	...
	<div class="flex min-w-[30rem] flex-1 flex-col gap-2 rounded-lg bg-gray-950 p-2">
			...
			<ul class="flex flex-col gap-2">
				{#each selectedUsers as user (user.id)}
					<li
						...
						title="Click to select account"
						class="flex cursor-pointer gap-4 rounded-md border border-gray-700 p-2 transition-all hover:bg-gray-600"
					>
						<Avatar.Root>
							<Avatar.Image src="" username={user.firstName} />
						</Avatar.Root>
