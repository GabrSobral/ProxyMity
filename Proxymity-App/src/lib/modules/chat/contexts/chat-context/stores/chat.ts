...
export const chatDispatch: Actions = {
					...
					conversation.messages = conversation.messages.map((message) => {
						if (!message.received.users.find((user) => user.userId === userId)) {
							...
							if (
								...
								message.received.users.filter((x) => x.at).length
							) {
	...
	addMessage({ message }) {
		...
		chatState.update((store) => {
			...
			if (index >= 0) {
				...
				// If the conversation is not selected, increment the notification
				const shouldNotification =
					store.conversations[store.selectedConversationIndex]?.id !== message.conversationId;
				const currentNotifications = store.conversations[index].notifications;

				store.conversations[index].notifications = shouldNotification
					? currentNotifications + 1
					: 0;

				// Remove the oldest message if the conversation has more than 100 messages
	...
	bringToTop(conversationId) {
		chatState.update((store) => {
			...
			// Sort the conversations array by the selected conversation and bring it to top
		...
		chatState.update((store) => {
			...
			if (conversation) {
				if (targetIndex > -1) {
					...
					store.conversations[targetIndex].messages = store.conversations[targetIndex].messages.map(
						(message) => {
			...
			const currentIndex = store.conversations.findIndex(
				(item) => item.id === store.conversations[store.selectedConversationIndex]?.id
			);
			if (currentIndex >= 0) {
				store.conversations[currentIndex].typeMessage = typeMessage;
			}

			store.selectedConversationIndex = targetIndex;
					...
					messages: conversation.lastMessages
						.map((message) => ({
							...
							author: {
								id: message.authorId,
								name:
									conversation.participants.find((item) => item.id === message.authorId)
										?.firstName || ''
							},
	...
	updateConversationMessageStatus(params) {
		...
		chatState.update((store) => {
			...
			if (conversationIndex > -1) {
				store.conversations[conversationIndex].messages = store.conversations[
					conversationIndex
